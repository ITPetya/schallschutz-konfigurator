...
// rechts, Y ist die Tiefe, Z ist die Höhe" - das WEICHT von three.js'
// eigener Achsbenennung ab (dort ist Welt-Y die echte Hoehe, siehe
// Container.tsx: H liegt auf Welt-Y; Welt-Z ist die Containerbreite/Tiefe).
// Hier deshalb bewusst NICHT a[1]/a[2] direkt mit "Y"/"Z" beschriftet,
// sondern gemaess Jonas' Bezeichnung getauscht: "Y" = Welt-Z-Differenz
// (Tiefe), "Z" = Welt-Y-Differenz (Hoehe) - siehe MeasureDimensions.tsx fuer
// dieselbe Zuordnung bei den In-Viewer-Bemaßungslinien.
function MeasureResultRows({ a, b, prefs }: { a: [number, number, number]; b: [number, number, number]; prefs: UnitPreferences }) {
  ...
  const depthM = Math.abs(a[2] - b[2]);
  const heightM = Math.abs(a[1] - b[1]);
  const directM = Math.hypot(a[0] - b[0], a[1] - b[1], a[2] - b[2]);

  return (
    <div className="space-y-1 text-xs">
      <MeasureRow label="X" meters={dxM} prefs={prefs} />
      <MeasureRow label="Y" meters={depthM} prefs={prefs} />
      <MeasureRow label="Z" meters={heightM} prefs={prefs} />
