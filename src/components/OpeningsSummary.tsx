...
export function OpeningsSummary({ openings }: OpeningsSummaryProps) {
  if (openings.length === 0) {
    return <p className="text-sm text-slate-400 dark:text-slate-500">Keine Durchbrüche platziert.</p>;
  }
  ...
  return (
    <div className="space-y-2">
      {openings.map((o) => {
        ...
        return (
          <div key={o.id} className="rounded-lg border border-slate-200 bg-white p-3 text-sm shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <div className="mb-2 flex items-center justify-between">
              <span className="font-medium text-brand-dark dark:text-brand-light">{typeDef.label}</span>
              <span className="text-xs text-slate-500 dark:text-slate-400">{PANEL_LABELS[o.panel]}</span>
            </div>
            <dl className="grid grid-cols-2 gap-x-2 gap-y-1 text-xs text-slate-600 dark:text-slate-300">
              <dt className="text-slate-400 dark:text-slate-500">{uLabel}</dt>
              <dd>{Math.round(o.u)} mm</dd>
              <dt className="text-slate-400 dark:text-slate-500">{vLabel}</dt>
              <dd>{Math.round(o.v)} mm</dd>
              <dt className="text-slate-400 dark:text-slate-500">{typeDef.shape === "round" ? "Durchmesser" : "Breite"}</dt>
              ...
              {o.hinge && (
                <>
                  <dt className="text-slate-400 dark:text-slate-500">Bandseite</dt>
